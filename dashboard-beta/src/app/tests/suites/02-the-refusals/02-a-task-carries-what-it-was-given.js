...
it('a task under a contract that is not approved is refused', async ({ okc, assert, log }) => {
  ...
  const refusal = await assert.refuses(
    () => okc('taskCreate', { task: { title: 'okc-test', brief: 'x', branch: 'inspection/check1', contractId: unapproved.id } }),
...
it('a task cannot name a job that does not exist', async ({ okc, assert, log }) => {
  const refusal = await assert.refuses(
    () => okc('taskCreate', { task: { title: 'okc-test', brief: 'x', branch: 'inspection/check1', job: 'no-such-job--test' } }),
    'no job called',
    'a task was written naming a job that is not there')
  log(`refused, and this is what it said:\n${refusal.message}`)
})

it('what a task was asked cannot change once it is out', async ({ okc, assert, log }) => {
  ...
  const refusal = await assert.refuses(
    () => okc('taskUpdate', { id: out.id, task: { brief: 'something else entirely' } }),
