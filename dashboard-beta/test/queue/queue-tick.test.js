...
//---- and a guard that unarmed itself, which is why it is not here ----------
//
//THE HAZARD WAS REAL: the queue defined `tasks` and, for a while, did not define
//`taskUpdate` -- so the tick READ this app's board and every write went down a
//relay to the app being ported from, the one actually running work on real
//machines. Adoption would read a stranded task here and re-queue it over there;
//a dispatch would mark `given` on a board it did not read it from.
//
//`ownsTheBoard` REFUSED TO DISPATCH AT ALL until both ends were the same app,
//and it was asked each tick rather than carried as a flag so that it would clear
//itself the day the action moved. It did, and then the relay went entirely --
//there is nowhere else for a write to land now, and no way to ask. Three tests
//covered it and went with it.
