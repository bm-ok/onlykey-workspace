...
module.exports = {
  ...
  vmDispatch: {
    ...
    run: async ({ name, task, folder, contract, rules: rulesText, contractName, resume, shell }) => {
      ...
      // A CONTINUATION SAYS SO. See `announcement` in tasks/sessions.js for what
      // it says and why, and note that this is only ONE of the two ways a brief
      // reaches a worker -- a task with a job goes through jobRun instead, and
      // the first version of this fix lived only here and therefore never fired.
      try {
        const doing = whatIsOn(name)
        const said = doing && sessions.announcement(doing)
        if (said) task = [said, '--- the task ---', '', String(task)].join('\n')
