...
draft('and a real run survives a real restart', [
  'THE OTHER HALF, and it needs somebody. A run is detached on purpose — nohup,',
  'its own session — so this app is not the work, and restarting it must',
  ...
  'had not dispatched.',
  '',
  'WHY IT IS NOT HERE: `adopt` runs once at startup, so proving this means',
  'stopping and starting the app around a machine that is genuinely mid-run.',
  'That is a person-driven drill, in the same family as the sign-in that needs',
  'somebody to visit a page.',
  '',
  'WHAT IS ALREADY COVERED WITHOUT IT: the rule that decides what is stranded,',
  'above, which is the half that was actually wrong.'
].join(' '))
